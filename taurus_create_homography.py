...
def main():
    ...
    selected_coords = np.array([[[0,0,0], [2,2,0], [1,5,0], [5,1,0], [4,3,0], [3,8,0]],
                       [[0,4,0], [2,1,0], [1,7,0], [4,3,0], [5,6,0], [3,7,0]]])

    world_points = selected_coords*0.0132
    ...
    # concatenate  right arm pose
    for i in range(12):

        with codecs.open("data/homography_taurus/poses_%s" % str(i+1), encoding = 'cp1252') as armfile:
            np.loadtxt( armfile, dtype=np.float, delimiter = " ")
            # data in microns and radians
            processed_pose = np.array(arm_pose).astype(np.float)
            print(processed_pose)
            processed_pose = processed_pose/1000 # transform to mm (radians are destroyed but we don't care)
            pose.append(processed_pose[15,19,23]) # append the pose of the right arm


    pose_matrix=np.array(pose)

    # print pose_left
    pose_matrix=np.transpose(pose_matrix)

    # create transformation from taurus to world
    M_right=affine_matrix_from_points(pose_matrix, world_points, scale=False, usesvd=True)

    r_L1_difference_yumi_to_world=[]
    r_L1_difference_world_to_yumi=[]
    point=1

    output1=taurus_to_world(pose_right,"right")
    output2=world_to_taurus(world_points,"right")

    a,b=output.shape

    for i in range(b):
    	r_L1_difference_yumi_to_world.append(numpy.linalg.norm((output1[:,i] - world_points[:,i]), ord=1))
    	r_L1_difference_world_to_yumi.append(numpy.linalg.norm((output2[:,i] - pose_matrix[:,i]), ord=1))

    # print "RIGHT L1 error yumi to world in mm",np.array(r_L1_difference_yumi_to_world)*1000
    print "RIGHT MEAN L1 error yumi to world in mm ------->",np.mean(np.array(r_L1_difference_yumi_to_world),axis=0)
    # print "RIGHT L1 error world to yumi mm",np.array(r_L1_difference_world_to_yumi)*1000
    print "RIGHT MEAN L1 error world to yumi in mm ------>",np.mean(np.array(r_L1_difference_world_to_yumi),axis=0)
    ...
    import pickle
    save_model = {'M_right':M_right}
    pickle_out = open("taurus_homography.pkl","wb")
