...
class Scene():
    ...
    def __init__(self,exec_model):
        ...
        self.drop_offset = [0.002,-0.001,0] 
        # grasp point params
        self.alpha = 0.75
        self.ksize = (3,3)
        ##### manual params ########
...
########################### End of all callback functions #########################
    ...
    def alternate_grasp_points(self, ROI, bbox, limb, K, offset=1):
        ...
        cv2.imwrite('Bin_Morphed.png', bw_img)
        cv2.waitKey(0)
        ...
        cv2.imwrite('AlternatePoints.png', ROI)
        cv2.waitKey(0)
        print('*'*50)
        ...
        cv2.imwrite("Grasppoint.png", self.color_frame)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
    ...
    for x in transfer_ids[0,18:]:
        ...
        for surgeme in sequence:
            ...
            if surgeme_no == 1:
                ...
                cv2.imwrite("NewROI.png", ROI_temp)
                cv2.waitKey(0)
