...
if __name__ == '__main__':
	...
	left_poles,right_poles = scene.pole_positions_rob(scene.poles_found) #Pole positions in robot space
	...
	while(surgeme_no < 8):
		...
		if surgeme_no == 1:
			while len(scene.pegs) == 0:#wait for pegs to be deteced
				# print("Enter")
				a = 1
			first_peg = np.array(scene.pegs[0]) #xmin,xmax,ymin,ymax choose peg closest to pole required 
