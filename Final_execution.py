...
########################### ENd of all callback functions #########################
	...
	def grasp_points(self,ROI,bbox,limb, offset=2):
		if limb=="left":
			offset=2
		if limb=="right":
			offset=2
	...
	def simple_drop_pose(self,ROI,limb,bbox,pole_pos):
	...
	def pole_positions_rob(self,pole_poses,robot):
		...
		if robot=="taurus":
			...
			# mythras claim
			# right_ids=[0,3,7,10,6,2]
			# left_ids = [1,5,9,11,8,4]
			left_ids=[0,3,7,10,6,2]
			right_ids = [1,5,9,11,8,4]
...
def S6(ROI,first_peg,drop_pose,limb):
	...
	drop_pt = scene.simple_drop_pose(ROI,limb,first_peg,drop_pose)
