...
########################### ENd of all callback functions #########################
	...
	def grasp_points(self,ROI,bbox,limb, offset=3):
	...
	def drop_pose(self,ROI,limb,bbox,pole_pos):
		...
		center_coord_x = ((rob_pose.translation[0])+(corner_robot_pose[0]-rob_pose.translation[0])/2.5)-0.005
		center_coord_y = (rob_pose.translation[1])+(corner_robot_pose[1]-rob_pose.translation[1])/2.5
	...
	def detect_ROI_approach(self,arm,selected_pole):
...
def S2(align,z_coord,K,limb):
	...
	print("is this sensible?:",yumi_pose)
	...
	while(surgeme_no < 8):
		...
		if surgeme_no == 1:
			...
			while len(scene.pegs) == 0:#wait for pegs to be deteced
				# print("Enter")
				a = 1
			selected_triangle_id=scene.detect_ROI_approach(limb,selected_pole)
		...
		if surgeme_no == 2:
			S2(grasp,scene.depth_vals[grasp[1],grasp[0]],scene.K,limb)#Perform Grasp
		...
		if surgeme_no == 6:
			...
			drop_triangle_id=scene.detect_ROI_drop()
			# selected_triangle_id=1
